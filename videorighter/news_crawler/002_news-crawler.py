...
def update_mongodb(hostname, port, username, password, database, data):
    client = MongoClient(hostname, port, username=username, password=password)
    db = client[database]

    # 각 필드별로 컬렉션에 데이터를 저장
    for item in data:
        db['title'].insert_one({'title': item['title']})
        db['date_news'].insert_one({'date_news': item['date_news']})
        db['summary'].insert_one({'summary': item['summary']})
        db['content'].insert_one({'content': item['content']})
        db['date_collect'].insert_one({'date_collect': item['date_collect']})
        db['url'].insert_one({'url': item['url']})
